"""t68_bf_panels.py -- t59 的五面板版本: 正文那一节的正确口径: 我们两步都用, Block Forests 用数据本身的七列.
...
用在相关块上. 全部 1000 棵树, 30000 训练行, 15000 测试行, 五张面板的全部季度, 一张面板一个进程.

    python t68_bf_panels.py NVDA
"""
import sys, time, warnings
from pathlib import Path
warnings.filterwarnings('ignore')
sys.path.insert(0, str(Path(__file__).resolve().parent))
sys.path.insert(0, '/Users/henry_han/LHP/JMP2+OptionAgent/DnET-MoJO/controls')
...
def main(tick='AAPL'):
    ...
    path = HERE / 'results' / f'bf_basic7_{tick}.csv'
    ...
    for i in range(4, len(qs)):
        ...
        for name, (p, dcol) in arms.items():
            ...
            rows.append(dict(tick=tick, fold=fold, model=name, d=dcol, M_blocks=M,
                             r2=r2_score(yte, p), mae=mean_absolute_error(yte, p)))
            line.append(f'{name} {rows[-1]["r2"]:.4f}')
        print(f'{tick} {fold}  M={M}/{dd}  ' + '  '.join(line) +
